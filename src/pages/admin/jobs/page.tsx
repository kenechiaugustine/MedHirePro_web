...
export default function AdminJobsPage() {
    ...
    const clinicOptions = users?.filter((u: any) => u.role === 'institute') || [];
    ...
    const handleCreateSubmit = async (e: React.FormEvent) => {
        ...
        const required_credentials = createForm.required_credentials_raw
            ...
            : [];
        const fringe_benefits = createForm.fringe_benefits_raw
            ? createForm.fringe_benefits_raw.split(',').map(b => b.trim()).filter(Boolean)
            : [];
        ...
        try {
            await postPermanentJob({
                ...
                accepts_interns: createForm.accepts_interns,
                rotation_schedule: createForm.rotation_schedule || null,
                fringe_benefits
            }).unwrap();

            toast.success("Permanent clinical job successfully published!");
            ...
            toast.error(err?.data?.detail || "Failed to publish standard job listing.");
        }
    };
    ...
    const handleReassign = async () => {
        ...
        if (newOwnerId === selectedJob.posted_by) {
            toast.error("Vacancy is already assigned to this host user.");
            return;
        }
        ...
        try {
            ...
            toast.success("Job posting successfully reassigned!");
    ...
    const handleDelete = async () => {
        if (!selectedJob) return;
        if (!window.confirm("Are you sure you want to delete this job posting? This action is permanent.")) return;
        ...
        <div className="space-y-6 animate-fadeIn duration-200">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div className="space-y-2">
                        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-teal-500/20 text-teal-100 text-xs font-semibold backdrop-blur-md border border-teal-500/20">
                            <FiShield className="w-3.5 h-3.5" /> Permanent Vacancies
                        </div>
                        <h1 className="text-2xl md:text-3xl font-black tracking-tight">Standard Job Listings</h1>
                        <p className="text-slate-255 text-xs md:text-sm font-medium opacity-90 leading-relaxed">
                            Supervisory directory of permanent job listings. Authorize updates, publish roles, and reassign vacancy ownership.
                        </p>
                    </div>
                    <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-white rounded-xl p-4 border border-slate-150 shadow-xs flex items-center justify-between">
                    <div>
                        <span className="text-[9px] font-black text-slate-400 tracking-wider uppercase block">Total Listings</span>
                        <span className="text-xl font-black text-slate-800">{filteredJobs.length} Positions</span>
                    </div>
                    <div className="w-10 h-10 rounded-lg bg-slate-50 text-slate-505 flex items-center justify-center text-lg"><FiBriefcase /></div>
                </div>
                <div className="bg-white rounded-xl p-4 border border-slate-150 shadow-xs flex items-center justify-between">
                    <div>
                        <span className="text-[9px] font-black text-emerald-400 tracking-wider uppercase block">Active Openings</span>
                        <span className="text-xl font-black text-emerald-700">{filteredJobs.filter(j => j.status === 'OPEN').length} Open</span>
                    </div>
                    <div className="w-10 h-10 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center text-lg"><FiBriefcase /></div>
                </div>
                <div className="bg-white rounded-xl p-4 border border-slate-150 shadow-xs flex items-center justify-between">
                    <div>
                        <span className="text-[9px] font-black text-indigo-400 tracking-wider uppercase block">Closed & Filled</span>
                        <span className="text-xl font-black text-indigo-700">{filteredJobs.filter(j => j.status === 'FILLED').length} Filled</span>
                    </div>
                    <div className="w-10 h-10 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center text-lg"><FiBriefcase /></div>
                </div>
            </div>
                ...
                {filteredJobs.length === 0 ? (
                    <div className="p-16 text-center space-y-4">
                        ...
                        <div className="space-y-1">
                            <h3 className="text-base font-extrabold text-slate-800">No Jobs Listed</h3>
                            <p className="text-xs text-slate-400 max-w-sm mx-auto font-medium">
                                No permanent clinical vacancies match your search parameters.
                            </p>
                        </div>
                    </div>
                ) : (
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-150 text-[10px] font-black text-slate-400 uppercase tracking-wider">
                                    ...
                                    <th className="px-6 py-4">Monthly/Hourly Compensation</th>
                                        ...
                                        >
                                            <td className="px-6 py-4.5 max-w-xs">
                                                <div className="space-y-0.5">
                                                    ...
                                                    <p className="text-[10px] text-slate-400 font-bold truncate flex items-center gap-1">
                                                        <FiUsers className="w-3 h-3 text-teal-600" /> {posterName}
                                                    </p>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4.5 whitespace-nowrap text-slate-450 font-semibold">
                                                <div className="flex items-center gap-1 text-[10px]">
                                                    <FiMapPin className="text-teal-600" /> {job.city}, {job.state}
                                                </div>
                                            </td>
                            ...
                            <div className="flex justify-between items-start">
                                <div className="space-y-1">
                                    <h3 className="text-base font-extrabold text-slate-800 flex items-center gap-1.5">
                                        <FiPlus className="text-teal-600" /> Publish Permanent Vacancy
                                    </h3>
                                    <p className="text-[10px] text-slate-400 font-semibold">
                                        Direct clinical role publication parameters.
                                    </p>
                                </div>
                                <button
                                    type="button"
                            ...
                            <div className="space-y-4">
                                <div className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        placeholder="e.g. Chief Cardiologist, Registered Midwife"
                                ...
                                <div className="space-y-1">
                                    ...
                                    <textarea
                                        ...
                                        placeholder="Full vacancy requirements and duties..."
                                        rows={3}
                                ...
                                <div className="grid grid-cols-3 gap-3 border-t border-slate-100 pt-4">
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Compensation Term</label>
                                        <select
                                ...
                                <div className="space-y-1">
                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Fringe Benefits (comma-separated)</label>
                                    <input
                                        type="text"
                                        value={createForm.fringe_benefits_raw}
                                        onChange={(e) => setCreateForm(prev => ({ ...prev, fringe_benefits_raw: e.target.value }))}
                                        placeholder="e.g. Free Housing, Relocation Bonus, Health Insurance"
                                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs outline-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-400 font-semibold text-slate-700"
                                    />
                                </div>
                                ...
                                <div className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        placeholder="e.g. MBChB, MD License, Pediatrics Board"
                                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs outline-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-400 font-semibold text-slate-700"
                                    />
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Rotation Schedule</label>
                                        <input
                                            type="text"
                                            value={createForm.rotation_schedule}
                                            onChange={(e) => setCreateForm(prev => ({ ...prev, rotation_schedule: e.target.value }))}
                                            placeholder="e.g. 3 weeks on / 1 week off"
                                            className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs outline-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-400 font-semibold text-slate-700"
                                        />
                                    </div>
                                    <label className="flex items-center justify-between p-3.5 bg-slate-50 rounded-xl border border-slate-150 cursor-pointer mt-5">
                                        <span className="text-[11px] font-extrabold text-slate-750">Accepts Interns</span>
                                        <input
                                            type="checkbox"
                                            checked={createForm.accepts_interns}
                                            onChange={(e) => setCreateForm(prev => ({ ...prev, accepts_interns: e.target.checked }))}
            ...
            {/* CONFIGURE PERMANENT JOB MODAL / REASSIGN OWNER */}
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 max-w-xl w-full overflow-hidden animate-slideUp duration-200">
                        ...
                        <div className="p-6 space-y-6 max-h-[85vh] overflow-y-auto">
                            <div className="flex justify-between items-start">
                                <div className="space-y-1">
                                    <h3 className="text-base font-extrabold text-slate-800 flex items-center gap-1.5">
                                        <FiSettings className="text-indigo-650" /> Configure Vacancy Ownership
                                    </h3>
                                    <p className="text-[10px] text-slate-400 font-semibold">
                                        Job Position: {selectedJob.position_title}
                                    </p>
                                </div>
                                <button
                                    type="button"
                            ...
                            <div className="bg-slate-50 border border-slate-150 rounded-xl p-4 space-y-3.5 text-xs text-slate-650">
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <span className="text-[9px] font-black uppercase text-slate-400 tracking-wider block">Vacancy Location</span>
                                        <p className="font-extrabold text-slate-850 pt-0.5">{selectedJob.city}, {selectedJob.state}</p>
                                    </div>
                                </div>
                                <div className="border-t border-slate-200/60 pt-3">
                                    <span className="text-[9px] font-black uppercase text-slate-400 tracking-wider block">Role Description</span>
                            ...
                            <div className="border-t border-slate-100 pt-4 space-y-4">
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide block">Select New Clinic / Host User</label>
                                    <select
                                        value={newOwnerId}
                                    ...
                                    >
                                        ...
                                        {clinicOptions.map((clinic: any) => (
                                            <option key={clinic._id} value={clinic._id}>
                                                {clinic.facility_name || clinic.full_name} ({clinic.email})
                                            </option>
                                        ))}
                                    </select>
                                </div>
                            ...
                            <div className="border-t border-slate-100 pt-4 space-y-3">
                                ...
                                >
                                    ...
                                    <FiTrash2 /> Delete Job Vacancy Listing
                                </button>
                            </div>
                        </div>
                    </div>
