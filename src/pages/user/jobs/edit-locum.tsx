...
export default function ProfessionalEditLocumJobPage() {
    const { jobId } = useParams<{ jobId: string }>();
    const navigate = useNavigate();

    const { data: job, isLoading: isJobLoading, error: jobError } = useGetJobListingDetailsQuery(jobId || '', { skip: !jobId });
    ...
    // Prepopulate form on mount or when data changes
    useEffect(() => {
        if (job) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.success("Locum job vacancy successfully updated!");
            navigate(USER_ROUTES.LOCUM_JOBS);
        } catch (err: any) {
            console.error(err);
            toast.error(err?.data?.detail || "Failed to update locum job vacancy.");
        }
    };
        ...
        return (
            <div className="flex h-96 items-center justify-center">
                <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-slate-500 font-medium text-xs">Retrieving shift details...</p>
                </div>
            </div>
        );
    }
    ...
    if (jobError || !job) {
        return (
            <div className="text-center py-12 bg-white rounded-2xl border border-slate-100 shadow-sm space-y-3 max-w-xl mx-auto">
                <div className="text-red-500 text-3xl">⚠️</div>
                <h3 className="font-extrabold text-slate-800">Failed to Load Listing</h3>
                <p className="text-xs text-slate-400 max-w-sm mx-auto font-medium">
                    The requested locum vacancy could not be found or you do not have permission to modify it.
                </p>
                <button
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-5">
                <div className="space-y-1">
                    ...
                    <h1 className="text-xl md:text-2xl font-black text-slate-800 flex items-center gap-2">
                        <FiClock className="text-teal-600" /> Edit Locum Placement
                    </h1>
                    <p className="text-xs text-slate-400 font-semibold">
                        Modify shift parameter info, rates, and coverage requirements.
                    </p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-slate-150 shadow-sm p-6 space-y-4">
                    <h3 className="font-extrabold text-slate-800 text-xs uppercase tracking-wider border-b border-slate-100 pb-3 flex items-center gap-1.5">
                        <FiBriefcase className="text-teal-500" /> Listing Publication Status
                    </h3>
                    <div className="space-y-1.5 max-w-xs">
                        <label className="text-[10px] font-bold text-slate-450 uppercase tracking-wide">Vacancy Status <span className="text-red-500">*</span></label>
                        <select
                            value={status}
                            onChange={(e) => setStatus(e.target.value as JobStatus)}
                            className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs outline-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-400 font-semibold text-slate-700 cursor-pointer"
                        >
                            <option value="OPEN">Open (Active Listings)</option>
                            <option value="DRAFT">Draft</option>
                            <option value="FILLED">Filled (Shift Assigned)</option>
                            <option value="EXPIRED">Expired / Deactivated</option>
                        </select>
                    </div>
                </div>
                
                {/* 2. SECTION: BASIC JOB INFO */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div className="space-y-1.5 md:col-span-2">
                            ...
                            <input
                                ...
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs outline-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-400 font-semibold text-slate-700"
                            />
                        </div>
                        ...
                        <div className="md:col-span-2 space-y-1.5">
                            ...
                            <textarea
                                ...
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs outline-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-400 font-semibold text-slate-700 leading-relaxed"
                            />
                        </div>
                    </div>
                </div>
                ...
                {/* 4. SECTION: RATES & SHIFT DETAILS */}
                <div className="bg-white rounded-2xl border border-slate-150 shadow-sm p-6 space-y-6">
                    <h3 className="font-extrabold text-slate-800 text-xs uppercase tracking-wider border-b border-slate-100 pb-3 flex items-center gap-1.5">
                        <FiBriefcase className="text-teal-555" /> 3. Shift Budget & Coverage parameters
                    </h3>
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                    ...
                    >
                        {isUpdating && <FiLoader className="animate-spin text-sm" />}
                        Save Changes
                    </button>
                </div>
            </form>
